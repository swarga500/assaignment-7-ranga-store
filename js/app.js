...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.classList.add("product",'col','text-center');
    div.innerHTML = `<div class="single-product card h-100">
    
    <div class="text-center mt-3">
    <img class="product-image " width="150px" height="150px" src=${product.image}></img>
    </div>
      <div class="card-body">
      <h3>${product.title}</h3>
      <p>Category: <span class="text-warning fw-bold">${product.category}</span></p>
      <h4>Rating: <span class="text-warning fw-bold">${product.rating.rate}</span>(${product.rating.count})
      <h2>Price: <span class="text-info">$ ${product.price}</span></h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now fw-bold btn btn-info">add to cart</button>
      <button id="details-btn" class="btn fw-bold">Details</button></div>
      </div>
    </div>
      `;
